...
namespace lsw::audio_diag::offline
{
    ...
    std::size_t WavReader::readBlock(std::vector<std::vector<double>>& channels, std::size_t maxFrames)
    {
        if (framesRead_ >= metadata_.frameCount) return 0;

        std::size_t framesToRead = std::min<std::size_t>(maxFrames, metadata_.frameCount - framesRead_);
        std::size_t bytesToRead = framesToRead * blockAlign_;
