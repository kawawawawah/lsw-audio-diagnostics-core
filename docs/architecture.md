...
2. `process()` sanitizes each supplied sample, updates fixed-capacity level and event trackers, and updates the stereo tracker when two channels are configured.
...
Snapshot publication uses a single-writer sequence guard plus individual 32-bit and 64-bit atomic fields. The writer marks the sequence odd, stores level, event, and stereo fields, then marks it even. Sequence and field operations use sequentially consistent ordering; a reader retries if its sequence changed or is odd.
